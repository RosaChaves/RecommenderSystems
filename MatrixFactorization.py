...
#as some data might be integer, use pickle library
...
# obtain the number of user2movie, movie2user and usermovie2rating obtained by collaborative filtering
...
# the loss is defined from the usermovie2rating. d: (user,movie)->rating
# the Loss is calculated from the rating prediction calculated from the tuple (user, movie) and the rating
def get_loss(d):
  N = float(len(d))
  sse = 0
  for k, r in d.items():
...
# train the parameters, taking 20 epochs
epochs = 20
